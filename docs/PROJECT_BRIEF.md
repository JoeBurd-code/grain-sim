...
- **`GrainFlowSim.jsx` retired; app renamed `MeetingApp` → `PlantApp` (2026-08-24).** The migration described in §3 row 13 and §4 above is complete — every piece of physics listed there has long since been lifted into `src/sim/`, and the mock had stopped being consulted as a reference. Removed the file and its lint exemption rather than keep carrying a frozen oracle nothing reads anymore. Renamed the top-level component to `PlantApp` since "MeetingApp" described its original meeting-prop origin (see the entry above) rather than its current job as the production app.
