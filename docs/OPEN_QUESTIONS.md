...
| Machine | Gap | Assumption in use | Expected from | Load bearing? |
|---|---|---|---|---|
| `treaterBufferBin` | LSH set point (supersedes the #18 row above, now that #19 reads it) | 85% of the 7.7 m³ working volume; live slider, `lineData.js` `interlocks[].highSetpoint`, `provenance: "assumed"` | **Nothing to absorb [FD 2026-08-05]**: operator-adjustable on the SCADA analog faceplate, so "assumed" is as good as it gets and the slider is the honest representation | No — the demo's point is that closing overshoots, not the exact percentage; dragging the slider is the presenter's answer to "what if the sensor were further upstream" |
| `treaterBufferBin` | LSL set point (supersedes the #18 row above) | 35% of working volume; live slider, `lineData.js` `interlocks[].lowSetpoint`, `provenance: "assumed"` | **Nothing to absorb [FD 2026-08-05]**, same reason | No, same reasoning as LSH |
| `bufferBinHighTrip` interlock | Signal delay — the real PLC scan interval plus any deliberate alarm debounce between the level switch tripping and the yellow-bin valve receiving a close command | **7 s (applied 2026-08-05)**; live slider, `lineData.js` `interlocks[].signalDelaySec`, `provenance: "confirmed"`, shared across every transition (open, close, trip) — see the LSHH row below | **ANSWERED [FD 2026-08-05]: ~7 s.** `LSH0` → **5 s** → elevator `52.414.E00` trips → **1 s** → vibratory feeders trip → **1 s** → bin outlet valves close (PLC_FD §5) — this figure describes the real trip chain, now modelled on LSHH (see below), and reused for the live LSH/LSL regulation by simplification, not because that regulation's own delay is independently confirmed | No — the demo's point is that overshoot grows with delay, not this specific value |
| `bufferBinHighTrip` interlock | ~~Reopen (low-trip) signal delay~~ ~~Superseded: whether an automatic reopen exists at all~~ **Resolved, issue #45; re-opened on request, see the LSHH row below** | Every *other* treating-zone interlock still latches per issue #45: a trip's commanded machine stays stopped once the level clears, and only RESET TRIPS (`src/sim/control.js` `resetTrips`) releases it. `bufferBinHighTrip` itself no longer works this way at LSH/LSL: on request it's now `hysteresisValve` (`src/sim/control.js`) — the valve closes at LSH and reopens on its own once the level clears LSL, live and non-latching, no operator action needed. The latch moved to a new LSHH instead — see the row below | **The FD's own answer (no automatic reopen) still applies to this tag's real trip signal** — LSH0 in the FD's own cause-and-effect matrix — **but the sim now models that trip on a different, unconfirmed instrument (LSHH)**, deliberately reintroducing a live, non-latching regulation at LSH/LSL on top. Not a re-litigation of the FD finding, a knowing departure from it, done on request | Not load-bearing for the demo's own point (a late, overshooting response), but genuinely load-bearing for "does this match the FD" — flagged, not silently reverted |
...
| `bufferBinHighTrip` interlock | LSHH trip set point — added on request so this valve's *latched* trip is re-sensored from LSH to a dedicated LSHH, matching the treater/Concetti pre-bins' own LSH/LSHH split (issue #58), while LSH/LSL themselves become a live, non-latching open/close regulation (the row above). The FD's own cause-and-effect matrix for this tag ties its one documented trip directly to `52.502.H00.LSH0`, naming no LSHH instrument and no live regulation beneath it | **95%, assumed**; live slider, `lineData.js` `interlocks[].highHighSetpoint`, `provenance: "assumed"`. This is the only instrument on this bin the FD's own "no automatic reopen" trip finding still applies to — crossing it commands the valve closed and only RESET TRIPS reopens it, gated on the level having actually cleared LSHH | Not FD-backed for this specific tag, on two counts now: which instrument traps (LSHH vs. the FD's own LSH0) and whether anything beneath it is live rather than latched. A deliberate, requested departure, not an oversight | No for the exact percentage (same reasoning as every other bin's LSHH); **yes** for the shape of the behaviour itself, since it now genuinely diverges from the FD's documented trip on this tag, unlike every other interlock on the line |
...
| `pendulumConveyor` (`52.604.E00`) | ~~Per-outlet transit distance~~ **Landed 2026-08-31, issue #69.** `routedTransportDelay` (`src/sim/behaviors.js`) now paces each packet against its own destination's own `portDistanceM` (`lineData.js`), not the shared whole-run `distanceM` | Still no document gives each outlet's own physical distance from the infeed, so the *split* is derived, not confirmed: each outlet's `portDistanceM` is the full-run `distanceM` (31.087 m, PDF-text exact) apportioned by that outlet's own anchor x-position against `outConcetti`'s (its own anchor sits at the physical tail of the drawn run, x:1745 of 1750, and is already the machine's real most-used destination — issue #56's `defaultPort`) — `outBuffer` (x:240) ≈4.28 m ≈25.4 s, `outBinSeg` (x:1120) ≈19.95 m ≈118.8 s, `outConcetti` unchanged at the full 31.087 m ≈185 s. `lineData.js`'s own `provenance.portDistanceM: "derived"` records this. A port with no `portDistanceM` entry (every routedTransportDelay machine except this one) falls back to the shared `distanceM`, unchanged | Engineer or the operational spec: each outlet's own along-belt distance from the infeed, to replace this derived split with a confirmed one | **Yes now** — this is the demo's own point per issue #69's acceptance criteria (outBuffer measurably sooner than outConcetti); a future confirmed figure could shift the three outlets' own relative timing, though not whether the effect is visible |
| `pendulumConveyor` (`52.604.E00`) | **Where the derived split puts each outlet physically.** Surfaced 2026-09-02 by issue #70 (the Z-shaped body). `distanceM`'s own 31.087 m is the *whole* carrying path — lower horizontal 7.084 m + vertical rise 9.157 m + upper horizontal 14.846 m — so the floor run and climb alone account for 16.241 m, i.e. sim-space fraction 0→0.522. But the issue #69 split above apportions by ceiling-run *anchor x* alone, which puts `outBuffer` at 4.28 m (fraction 0.138) and `outBinSeg` at 19.95 m (0.642) — and 4.28 m lands **inside the floor run**, before the climb has even started, for an outlet that is physically on the ceiling run. The three outlets' own *relative order and spacing* are still right; their absolute positions along the path are not | Left exactly as issue #69 derived it — the numbers are `derived`, already flagged in the row above, and correcting them would re-time real deliveries (`outBuffer` would go from ≈25 s to ≈108 s) on the strength of another assumption rather than a confirmed figure. The render no longer depends on them for *where* grain is drawn to stop: `outletPathFraction` (`src/scene/elevatorMotion.js`) takes that from the outlet's own drawn anchor instead. Note the drawn path's own segment proportions (10/27/63%) do not match the real machine's (23/29/48%) either, so sim-space and drawn-space fractions are **not** interchangeable in either direction | Engineer or the operational spec: the same figure the row above wants — each outlet's own along-belt distance from the infeed. That single answer resolves both rows | **Timing only.** Visibly, grain now stops at the drawn outlet regardless (issue #70 follow-up); what stays derived is *how long* material bound for the two nearer outlets takes to get there |
