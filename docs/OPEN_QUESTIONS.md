...
| `flexiconPreBin` | LSH0/LSL0 set points, demo starting fill level (45%) | 85% / 35% / 45%; live sliders, `lineData.js` `sim.initialLevelFraction` and `interlocks[].lowSetpoint`, `provenance: "assumed"` | **Nothing to absorb [FD 2026-08-05]**, same reasoning as every other bin on the line: operator-adjustable SCADA configuration | No, same reasoning as every other bin's LSH/LSL |
...
| `concettiPreBin` | LSH0/LSL0 set points, demo starting fill level (50%) | 85% / 35% / 50%; live sliders, `lineData.js` `sim.initialLevelFraction` and `interlocks[].lowSetpoint`, `provenance: "assumed"` | **Nothing to absorb**, same reasoning as every other bin on the line: operator-adjustable SCADA configuration | No, same reasoning as every other bin's LSH/LSL |
