...
| Machine | Gap | Assumption in use | Expected from | Load bearing? |
|---|---|---|---|---|
| `treatDrumFeeder` | Percentage-opening → flow-rate mapping. The engineer was explicit the real drum feeder is not proportional — ten percent opening does not mean ten percent of range, and forty percent might land around 12 t/h — and referred to a spreadsheet of estimated values that was never sent | Linear opening → rate mapping assumed across the confirmed 2-20 t/h range; the sim exposes the feeder as a direct rate control rather than a percentage dial. Documented in `lineData.js`'s `treatDrumFeeder` comment (not a numbered field, so not under `sim.provenance`) | The engineer's spreadsheet of estimated values (referenced, not yet sent) | **Yes** — this is the first genuinely load-bearing gap on the line: the actual shape of the opening→rate curve changes where on the dial a given real-world tonnage sits, which is exactly what a presenter would be asked about live |
| `treatDrumFeeder` | Start condition — the engineer confirmed the real feeder starts as soon as the bucket elevator is confirmed running | Not modelled: the elevator isn't sim-enabled yet (issue #21+), so the feeder starts at 0 (off) and the presenter starts it live via the rate slider, same staging pattern as issue #19's level slider | Neither — mechanically deferred until the elevator interlock exists, not a plant fact in question | No — the demo's point for #20 is the fill/draw balance, not the auto-start trigger; revisit once the elevator lands |
