...
| `treaterBufferBin` | Working-volume→mass conversion, needed the moment any readout is in tonnes | **Answered [FD 2026-08-05]: 0.72 t/m³**, from three bins the vendor rated in both m³ and tonnes (agreeing within 1%). Not yet used in code | Resolved | Was yes, now closed |
...
| `bufferBinHighTrip` interlock | Signal delay — the real PLC scan interval plus any deliberate alarm debounce between the level switch tripping and the yellow-bin valve receiving a close command | 3 s; live slider, `lineData.js` `interlocks[].signalDelaySec`, `provenance: "assumed"` | **ANSWERED [FD 2026-08-05]: ~7 s.** `LSH0` → **5 s** → elevator `52.414.E00` trips → **1 s** → vibratory feeders trip → **1 s** → bin outlet valves close (PLC_FD §5). Default should move to 7 s and provenance to `confirmed` | No — the demo's point is that overshoot grows with delay, not this specific value; but the default is now wrong by half, so it is worth fixing |
...
| `topConveyor` | Whether it exists as a separate machine at all, or is the upper horizontal run of pendulum conveyor `52.604.E00` with pneumatically selected outlets | Modelled as a distinct conveyor `52.605.X00`, a tag the FD reassigns to the Concetti auto sampler | Engineer, one line. See `PLC_FUNCTIONAL_DESCRIPTION.md` §8.3 | **Yes for the scene**, no for the flow: it changes what is drawn and how the three branches are labelled, not how material moves |
| `concettiMetalRemover` | Whether it exists | Present in `lineData.js` with a waste stub | **Resolved: it does not exist [FD 2026-08-05].** Two independent sources now agree with the engineer against the sheet 52-14 cross-reference | No, just delete it |
