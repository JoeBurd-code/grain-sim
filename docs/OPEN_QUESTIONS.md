...
| `treaterAfterBin` | No live discharge yet: the scalping screen (`52.602.F00`) downstream isn't sim-enabled (it's a splitter, per the parent spec's behaviour census, built in a later issue), so the after-bin currently only accumulates toward its own high set point in the running app — same situation the buffer bin was in before the drum feeder (#20) existed | Not modelled: the engine's reverse-pass capacity check already guarantees this can never cause a spill regardless (`src/sim/engine.js` `stepSim`), and the sawtooth-under-pulsed-infeed-with-steady-draw property the acceptance criteria ask for is proven directly against the reused accumulator behaviour in `src/sim/behaviors.test.js`, not against the live (currently downstream-less) line | Nothing to absorb — resolved automatically once the scalping screen's own issue lands and wires a real downstream | No |
