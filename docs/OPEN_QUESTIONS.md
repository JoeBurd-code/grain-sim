...
| `bufferBinHighTrip` interlock | LSHH trip set point — added on request so this valve's trip is re-sensored from LSH to a dedicated LSHH, matching the treater/Concetti pre-bins (issue #58). The FD's own cause-and-effect matrix for this tag ties the trip directly to `52.502.H00.LSH0`, naming no LSHH instrument here | **95%, assumed**; live slider, `lineData.js` `interlocks[].highHighSetpoint`, `provenance: "assumed"`. LSH (85%) drops to display-only alongside LSL, joining the FD's own "no automatic reopen" latch — see the row above | Not FD-backed for this specific tag — a deliberate consistency choice (one shared trip-set-point convention across every bin) rather than a plant fact; nothing further to ask the engineer since the underlying trip behaviour (closes late, overshoots, latches until reset) is unchanged, only which instrument arms it | No — same reasoning as the LSH/LSL rows above: the demo's point is the overshoot and the latch, not the exact percentage or which named switch triggers it |
